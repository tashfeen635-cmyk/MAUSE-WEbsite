...
async function handler(req, res) {
  ...
  try {
    ...
    try {
      await connectDB();
      console.log('MongoDB connected successfully for projects API');
    } catch (dbError) {
    ...
    if (req.method === 'GET') {
      try {
        ...
        console.error('Projects query error:', queryError);
        jsonResponse(500, {
    ...
    if (req.method === 'POST') {
      ...
      if (!title || !description || !image) {
        jsonResponse(400, { message: 'Title, description, and image are required' });
        return;
      }
